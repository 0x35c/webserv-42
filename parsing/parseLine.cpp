...
void	parseLineLocationBlock(const std::string & line, const int & nbLine, t_location & location)
{
	...
	void (*argumentsFunctions[8])(t_location &, const int &, const std::vector<std::string> &) = {&methodAttribute, &returnAttribute, &rootAttribute, &directoryListingAttribute, &indexAttribute, &acceptUploadedFileAttribute, &saveUploadedFileAttribute};
	std::vector<std::string> lineSplitted = splitString(line, ' ');
